...
export default async function handler(req, res) {
    ...
    // सुनिश्चित करें कि सभी ज़रूरी डेटा मौजूद है
    if (!text || !language || !audioDataUrl) {
        return res.status(400).json({ error: 'Missing required parameters: text, language, or audioDataUrl.' });
    }
    ...
    try {
        ...
        const startResponse = await fetch('https://api.replicate.com/v1/predictions', {
            ...
            body: JSON.stringify({
                // यह Coqui XTTS v2 वॉइस क्लोनिंग मॉडल का सही वर्जन है
                version: "3495203c3b06c8889a721342a781033230a1334c2a412cb591a6202479e0a6d5",
                input: {
                    text: text,
                    speaker_wav: audioDataUrl, // यह Base64 डेटा URL या एक पब्लिक URL हो सकता है
                    language: language,
                },
            }),
        });
        ...
        if (startResponse.status !== 201) {
            ...
            return res.status(500).json({ error: startData.detail || 'Failed to start prediction job.' });
        }

        const predictionUrl = startData.urls.get;
        let finalResult = null;

        // जब तक परिणाम "succeeded" या "failed" नहीं हो जाता, तब तक हर 2 सेकंड में चेक करें
        while (!finalResult || (finalResult.status !== 'succeeded' && finalResult.status !== 'failed')) {
            ...
            if (finalResult.status === 'failed') {
                return res.status(500).json({ error: finalResult.error || 'Prediction job failed.' });
            }
        }
        
        // अगर जॉब सफल हो जाती है, तो ऑडियो का URL वापस भेजें
        // Replicate का आउटपुट एक ऐरे (array) होता है, जिसमें पहला आइटम ऑडियो URL होता है
